...
def get_recorded_vars(layers,num_classes,num_timesteps,dt):
    """Retrieve neuron variables recorded during simulation.
    For each time step, get number of spikes of all neurons in the output layer.
    """
...
f = open(layers_path+'/results_log',"w+")
...
#        mem_pot = layers[1].get_data().segments[-1].analogsignals
    ...
    #pro.get('weight')
    # get spiketrains
    output = get_recorded_vars(layers,num_classes,num_timesteps,dt)
    # print (str(output))
    ...
    log = 'ClassID: ' + str(results[batch_idx][0]) + ', top-1: ' + str(top1)+ '\n'
