...
cellparams = {'v_thresh': v_th, 'v_reset': 0.0, 'v_rest': 0.0, 'e_rev_E': 10.0, 'e_rev_I': -10.0, 'i_offset': 0.0,
              'cm': 0.001, 'tau_m': 0.8325, 'tau_syn_E': 0.01, 'tau_syn_I': 0.01, 'tau_refrac': 0.0}
...
for layer in range(len(weights)):
    ...
    sim.Projection(pops[layer],pops[layer+1], connector=sim.FromListConnector(inh_synapses),
                    receptor_type='inhibitory')
    sim.Projection(pops[layer],pops[layer+1], connector=sim.FromListConnector(exc_synapses),
...
for sample_idx in range(num_to_test):
    ...
    membranes_1 = pops[1].get_data().segments[-1].analogsignals[0]
